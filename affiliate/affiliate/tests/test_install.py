...
class TestInstall(UnitTestCase):
	...
	The hook provisions the "Affiliate" Role that the registration
	flow (affiliate.api.registration.register_affiliate) assigns to
	every new affiliate user. Before the hook existed, the role was
	referenced by name but never created anywhere - so the very first
	self-registration on a fresh install threw "Could not find Row #1:
	Role: Affiliate" inside User.save() and rolled back, blocking
	signups entirely until an admin manually created the role.
